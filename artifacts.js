import {LANGUAGE} from "../language";

export const artifact_data = [
    {
        name: "adventurer",
        fullname: {
            en: "Adventurer"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_midsummer_courtyard"
            },
        ]
    },
    {
        name: "archaic_petra",
        fullname: {
            en: "Archaic Petra"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_domain_of_guyun"
            },
        ]
    },
    {
        name: "berserker",
        fullname: {
            en: "Berserker (Blood-Soaked)"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "world_boss"
            },
        ]
    },
    {
        name: "bloodstained_chicalry",
        fullname: {
            en: "Bloodstained Chivalry"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_clear_pool_and_mountain_cavern"
            },
        ]
    },
    {
        name: "brave_heart",
        fullname: {
            en: "Brave Heart"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_domain_of_guyun"
            },
        ]
    },
    {
        name: "crimson_witch_of_flames",
        fullname: {
            en: "Crimson Witch of Flames"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_hidden_palace_of_zhou_formula"
            },
        ]
    },
    {
        name: "defender_will",
        fullname: {
            en: "Defender's Will"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_hidden_palace_of_zhou_formula"
            },
        ]
    },
    {
        name: "gambler",
        fullname: {
            en: "Gambler"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_clear_pool_and_mountain_cavern"
            },
        ]
    },
    {
        name: "glacier_and_snowfield",
        fullname: {
            en: "Glacier and Snowfield"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "world_treasure"
            },
        ]
    },
    {
        name: "gladiator_finale",
        fullname: {
            en: "Gladiator's Finale"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "world_boss"
            },
        ]
    },
    {
        name: "ice_breaker",
        fullname: {
            en: "Icebreaker"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "world_treasure"
            },
        ]
    },
    {
        name: "instructor",
        fullname: {
            en: "Instructor"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "world_boss"
            },
        ]
    },
    {
        name: "lavawalker",
        fullname: {
            en: "Lavawalker"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_hidden_palace_of_zhou_formula"
            },
        ]
    },
    {
        name: "lucky_dog",
        fullname: {
            en: "Lucky Dog"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_domain_of_guyun"
            },
        ]
    },
    {
        name: "maiden_beloved",
        fullname: {
            en: "Maiden Beloved"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_valley_of_remembrance"
            },
        ]
    },
    {
        name: "martial_artist",
        fullname: {
            en: "Martial Artist"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_hidden_palace_of_zhou_formula"
            },
        ]
    },
    {
        name: "noblesse_oblige",
        fullname: {
            en: "Noblesse Oblige"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_clear_pool_and_mountain_cavern"
            },
        ]
    },
    {
        name: "resolution_of_sojourner",
        fullname: {
            en: "Resolution of Sojourner"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_midsummer_courtyard"
            },
        ]
    },
    {
        name: "retracing_bolide",
        fullname: {
            en: "Retracing Bolide"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_domain_of_guyun"
            },
        ]
    },
    {
        name: "scholar",
        fullname: {
            en: "Scholar"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_clear_pool_and_mountain_cavern"
            },
        ]
    },
    {
        name: "the_exile",
        fullname: {
            en: "The Exile (Legacy)"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "world_boss"
            },
        ]
    },
    {
        name: "thundering_fury",
        fullname: {
            en: "Thundering Fury"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_midsummer_courtyard"
            },
        ]
    },
    {
        name: "thundersoother",
        fullname: {
            en: "Thundersoother"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_midsummer_courtyard"
            },
        ]
    },
    {
        name: "tiny_miracle",
        fullname: {
            en: "Tiny Miracle"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_valley_of_remembrance"
            },
        ]
    },
    {
        name: "traveling_doctor",
        fullname: {
            en: "Traveling Doctor (Curative)"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_valley_of_remembrance"
            },
        ]
    },
    {
        name: "viridescent_venerer",
        fullname: {
            en: "Viridescent Venerer"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "domain_blessing_valley_of_remembrance"
            },
        ]
    },
    {
        name: "wanderer_troupe",
        fullname: {
            en: "Wanderer's Troupe"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "world_boss"
            },
        ]
    },

    {
        name: "prayers_of_illumination",
        fullname: {
            en: "Prayers for Illumination"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "pyro_regisvines",
            },
        ]
    },
    {
        name: "prayers_to_springtime",
        fullname: {
            en: "Prayers to Springtime"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "cryo_regisvines",
            },
        ]
    },
    {
        name: "prayers_for_wisdom",
        fullname: {
            en: "Prayers for Wisdom"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "electro_hypostasis",
            },
        ]
    },
    {
        name: "prayers_for_destiny",
        fullname: {
            en: "Prayers for Destiny"
        },
        item_list: [
            {
                name: "Artifact",
                farm_at: "oceanid",
            },
        ]
    },
]

export const artifact_map = artifact_data.reduce((acc, cur) => ({...acc, [cur.name]: cur}), {});

export const artifact_option = artifact_data.map((artifact) => ({
    value: artifact.name,
    label: artifact.fullname[LANGUAGE],
}));

export const artifact_material = artifact_data.reduce((acc, cur) => ([...acc, ...(cur.item_list.map((titem, i) => ({name: `artifact_${cur.name}_${i}`, fullname: `${cur.fullname[LANGUAGE]} ${titem.name}`, farm_at: titem.farm_at, is_artifact: true, artifact: cur,})))]), []);